...
export default function CommoditiesDashboard({ initialData }: { initialData: any[] }) {
  ...
  const [formData, setFormData] = useState({ name: '', ratePerMtMonth: 0 });
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Rate (₹/MT/Month)</label>
            <Input 
              required 
              type="number" 
              value={formData.ratePerMtMonth} 
              onChange={(e) => setFormData({ ...formData, ratePerMtMonth: Number(e.target.value) })} 
              placeholder="12.50"
            />
          </div>
        </div>
        <div className="flex justify-end gap-2">
          {editingId && (
            <Button variant="outline" type="button" onClick={() => { setEditingId(null); setFormData({ name: '', ratePerMtMonth: 0 }); }}>
              Cancel
            </Button>
          )}
          <Button type="submit">
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50">
              ...
              <TableHead className="font-bold">Monthly Rate (₹/MT)</TableHead>
              ...
              <TableRow key={item._id}>
                ...
                <TableCell>
                  <span className="font-mono text-indigo-700 font-bold bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">
                    ₹{item.ratePerMtMonth.toFixed(2)}
                  </span>
                </TableCell>
                <TableCell className="text-right space-x-2">
                  <Button variant="ghost" size="icon" onClick={() => { setEditingId(item._id); setFormData({ name: item.name, ratePerMtMonth: item.ratePerMtMonth }); }}>

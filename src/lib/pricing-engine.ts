 ...
 *   = (10 × 90 × 20) / 30
 *   = ₹600.00 (before 18% GST → ₹708.00 total)
 *
...
export interface RentBreakdown {
  ...
  storageRent:  number;   // dailyRate × totalDays (pre-tax subtotal)
  gstAmount:    number;   // 18% GST on storageRent
  totalAmount:  number;   // storageRent + gstAmount
}
...
export function calculateRent(
  ...
  const gstPaise      = Math.round(rentPaise * GST_RATE);        // rounded integer
  const totalPaise    = rentPaise + gstPaise;

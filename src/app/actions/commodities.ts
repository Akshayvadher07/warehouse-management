...
export async function fetchCommodities() {
  ...
  return JSON.parse(JSON.stringify(items.map(item => ({
    _id: item._id,
    name: item.name,
    ratePerMtMonth: item.ratePerMtMonth,
    updatedAt: item.updatedAt,
    createdAt: item.createdAt,
  }))));
}

export async function addCommodity(data: { name: string; ratePerMtMonth: number }) {
  await connectToDatabase();
  try {
...
export async function updateCommodity(id: string, data: { name: string; ratePerMtMonth: number }) {
  await connectToDatabase();
  try {
